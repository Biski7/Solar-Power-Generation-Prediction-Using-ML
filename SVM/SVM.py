...
param_grid = [{"svc__C":C, 'svc__kernel':['linear']}, {'svc__C': C, 'svc__gamma': gamma, 'svc__kernel': ['rbf']}]
...
gs = GridSearchCV(estimator=pipe_svc, param_grid=param_grid, scoring='accuracy',cv=cross_validation, n_jobs=-1, refit=True, return_train_score=True )
...
best_param_C = best_param['svc__C']
best_param_kernel = best_param['svc__kernel']
best_param_gamma = best_param['svc__gamma']
y_pred = cross_val_predict(pipe_svc, X_test_lda, y_test)
...
with open("BT_SVM.txt", "w") as f:
  f.write('Best Score: %.2f' %best_score)
  f.write('\n')
  f.write('Best Param C: %.5f '  %best_param_C) 
  f.write('\n')
  f.write('Best Param kernel: %.5f '  %best_param_kernel) 
  f.write('\n')
  f.write('Best Param gamma: %.5f '  %best_param_gamma) 
  f.write('\n')
