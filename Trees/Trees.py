...
pipe_decision_tree = make_pipeline( DecisionTreeClassifier(random_state=1))
...
param_grid = {"decisiontreeclassifier__max_depth":max_depth, 'decisiontreeclassifier__criterion':['gini', 'entropy']}
...
gs = GridSearchCV(estimator=pipe_decision_tree, param_grid=param_grid, scoring='accuracy',cv=cross_validation, n_jobs=-1, refit=True, return_train_score=True )
...
best_param_max_depth = best_param['decisiontreeclassifier__max_depth']
best_param_criterion = best_param['decisiontreeclassifier__criterion']
y_pred = cross_val_predict(pipe_decision_tree, X_test_lda, y_test)
...
#
######################################################
print(' True Training Accuracy: %.5f' % gs.score(X_train_lda, y_train))
print('\n')
print(' True Test Accuracy: %.5f' % gs.score(X_test_lda, y_test))
print('\n')
...
with open("BT_Trees.txt", "w") as f:
  f.write('Best Score: %.5f' %best_score)
  f.write('\n')
  f.write('Best Param max depth: %.5f '  %best_param_max_depth) 
  f.write('\n')
  f.write('Best Param criterion: %.8s '  %best_param_criterion) 
  f.write('\n')
...
plt.plot(fpr[0], tpr[0], linestyle='-',color='red', label='0-100')
...
plt.plot(fpr[1], tpr[1], linestyle='--',color='green', label='3K-4K')
