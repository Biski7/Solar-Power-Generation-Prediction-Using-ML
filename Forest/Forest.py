...
estimators = [2,3,4,5,6,7,8,9,10,12,14,16,18,20,24]
...
param_grid = {"randomforestclassifier__n_estimators":estimators, 'randomforestclassifier__criterion':['gini', 'entropy']}
...
gs = GridSearchCV(estimator=pipe_forest, param_grid=param_grid, scoring='accuracy',cv=cross_validation, n_jobs=-1, refit=True, return_train_score=True )
...
best_param_n_estimators = best_param['randomforestclassifier__n_estimators']
best_param_criterion = best_param['randomforestclassifier__criterion']
y_pred = cross_val_predict(pipe_forest, X_test_lda, y_test)
...
with open("BT_Forest.txt", "w") as f:
  f.write('Best Score: %.5f' %best_score)
  f.write('\n')
  f.write('Best Param number of estimators: %.5f '  %best_param_n_estimators) 
  f.write('\n')
  f.write('Best Param criterion: %.5s '  %best_param_criterion) 
  f.write('\n')
...
plt.plot(fpr[1], tpr[1], linestyle='--',color='black', label='3K-4K')
